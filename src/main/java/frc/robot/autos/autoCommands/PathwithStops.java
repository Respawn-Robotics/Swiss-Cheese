...
public class PathwithStops extends SequentialCommandGroup {
    public PathwithStops(Swerve s_Swerve,ArmSubsystem armSubsystem,WristSubsystem wristSubsystem, CollectionSubsystem collectionSubsystem, Vision limelightSubsystem){
...
eventMap.put("marker1", wristSubsystem.setVoltage(-.1f).andThen(new WaitCommand(.8).andThen(wristSubsystem.setVoltage(0).andThen(wristSubsystem.resetPos()))));
...
eventMap.put("intakeDown", armSubsystem.setPosition(Constants.ArmConstants.ACQUIRE_FROM_CUBE_FLOOR).andThen(wristSubsystem.setPosition(Constants.WristConstants.ACQUIRE_FROM_CUBE_FLOOR).andThen(collectionSubsystem.collectCube())));//.andThen(new WaitCommand(1.5)).andThen(collectionSubsystem.collectCone()).andThen(new PrintCommand("workplz").andThen(new WaitCommand(.5).andThen(armSubsystem.setPosition(0)).andThen(wristSubsystem.setPosition(0).andThen(collectionSubsystem.stopMotor()))))));
